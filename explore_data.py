...
def main(args):
    ...
    mvct_images_paths, mvct_convhull_paths = clean_annotations(mvct_images_paths, mvct_convhull_paths)

    # Planning images and convex slices
    # print(io.loadmat(CT_PATH + '/imageP.mat')
    ct_images = io.loadmat(CT_PATH + '/imageP.mat')['imageP']
    ct_convex = io.loadmat(CT_PATH + '/rectumP_man_seg_slice_convhull.mat')['rectumP_seg_man']
    ...
    if int(args[1]) == 0:
        ...
        for i in range(mvct_convhull.shape[2]):
            ...
            # left = N//2 - 128
            # right = N//2 + 128
            resized_mvct = cv2.resize(mvct_image[:, :, i], (256, 256), cv2.INTER_LANCZOS4)
            resized_mvct_mask = cv2.resize(mvct_convhull[:, :, i], (256, 256), cv2.INTER_NEAREST)
        ...
        for i in range(ct_convex.shape[2]):
            ...
            resized_ct_mask = cv2.resize(ct_convex[:, :, i], (512, 512), cv2.INTER_NEAREST)
